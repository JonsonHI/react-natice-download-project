...
* priority: 图片下载优先级: LHTImagePlaceholder.priority.normal (low, high)
* cache: 下载缓存策略: LHTImagePlaceholder.cacheControl.immutable (web, cacheOnly)
* resizeMode: 图片显示类型: LHTImagePlaceholder.resizeMode.contain (cover, stretch, center)
* func:
 ...
 */
export default class LHTImagePlaceholder extends Component{
    ...
    constructor(props) {
        ...
        //下载状态值
        this.status = LHTImagePlaceholder.Status.NoDown;
    }
    ...
    * */
    ...
    //开始下载网络图片
    _onLoadStart = () => {
        this.status = LHTImagePlaceholder.Status.isDownLoading;
        if (this.props.onLoadStart) this.props.onLoadStart();
    }

    //下载成功
    _onLoad = (event) => {
        this.status = LHTImagePlaceholder.Status.DownSuccess;
        this.forceUpdate();
    ...
    //下载失败
    _onError = () => {
        this.status = LHTImagePlaceholder.Status.DownFail;
        this.forceUpdate();
    ...
    render() {
        ...
        //默认图
        let defaultSource = this.props.defaultSource ? this.props.defaultSource : xpxImg.defaultProject;
        //图片样式
        ...
        if (status == LHTImagePlaceholder.Status.NoDown) {
            showLoading = false;
        } else if (status == LHTImagePlaceholder.Status.isDownLoading) {

        } else if (status == LHTImagePlaceholder.Status.DownSuccess) {
            shwoDefaultImage = false;
            showLoading = false;
        } else if (status == LHTImagePlaceholder.Status.DownFail) {
            showLoading = false;
        }
